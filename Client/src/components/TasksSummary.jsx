...
export default function TasksSummary() {
  ...
  const summaryCards = [
    { title: 'My Tasks', count: myTasks.length, icon: User, color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-400', items: myTasks.slice(0, 3) },
    { title: 'Overdue', count: overdueTasks.length, icon: AlertTriangle, color: 'bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-400', items: overdueTasks.slice(0, 3) },
    { title: 'In Progress', count: inProgressTasks.length, icon: Clock, color: 'bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-400', items: inProgressTasks.slice(0, 3) },
  ];
          ...
          <div className="border-b border-zinc-200 dark:border-zinc-800 p-4 pb-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center justify-between flex-1">
                ...
                <span className={`px-2 py-1 rounded text-xs font-semibold ${card.color}`}>{card.count}</span>
              </div>
            </div>
          </div>
          <div className="p-4">
              ...
              <div className="space-y-3">
                {card.items.map((task) => (
                  <div key={task.id} className="p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer">
                    ...
                    <p className="text-xs text-gray-600 dark:text-zinc-400 capitalize mt-1">{task.type?.toLowerCase()} • {task.priority?.toLowerCase()} priority</p>
                  </div>
                ))}
                {card.count > 3 && (
